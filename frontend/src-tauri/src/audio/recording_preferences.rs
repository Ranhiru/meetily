...
pub async fn get_available_audio_backends() -> Result<Vec<String>, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        Ok(backends.iter().map(|b| b.as_str().to_string()).collect())
    }
...
pub async fn get_audio_backend_info() -> Result<Vec<BackendInfo>, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let backends = vec![
            BackendInfo {
                id: AudioCaptureBackend::ScreenCaptureKit.as_str().to_string(),
            ...
            BackendInfo {
                id: AudioCaptureBackend::CoreAudio.as_str().to_string(),
